...
baselin_OLINK$Responder <- factor(baselin_OLINK$NASH)
...
p_all <- apply(OLINK[1:(length(OLINK)-1)], 2, function(x) wilcox.test(x ~ OLINK$NASH))
...
OLINK_training <- OLINK

OLINK_training$NASH <- factor(OLINK_training$NASH)
OLINK_validation$NASH <- factor(OLINK_validation$NASH)

OLINK_training <- OLINK_training[which(colnames(OLINK_training) %in% c(row.names(p_all),"NASH"))]
OLINK_validation <- OLINK_validation[which(colnames(OLINK_validation) %in% c(row.names(p_all),"NASH"))]
...
pred_result = data.frame(pred_result, OLINK_validation$NASH)
pred_result$obs <- pred_result$OLINK_validation.NASH
